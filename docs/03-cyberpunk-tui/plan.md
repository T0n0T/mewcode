...
├── interactive TTY
│   └── CyberpunkChatApp
│       ├── Conversation / Composer / SessionFooter / Overlay
...
- `SessionFooter`：在输入区下方以单行显示品牌、模型、工作区、Git 分支和连接状态。
- `ActivityIndicator`：`UPLINKING`、`EXECUTING`、`SYNTHESIZING` 和计时。
- `ConfirmationModal`：危险操作预览与批准/拒绝。
- `NewOutputIndicator`：用户离开底部后的新输出提示。

正常尺寸下，应用按照 `ConversationView` → `NewOutputIndicator` → `#composer-shell` → `SessionFooter` 的固定垂直顺序装配；`#size-warning` 作为默认隐藏的最后一个节点存在。`Screen` 统一提供四边各一格的外层 padding，所有正常内容均位于该安全区域内。

`SessionFooter` 与输入区使用连续的 `$surface` 背景。输入区保留顶部青色分隔线，Footer 自身不再添加上下边框，避免两个相邻区域出现双边框。尺寸过小时隐藏对话区、新输出提示、输入区和 Footer，只显示尺寸提示。
...
`ActivityState` 只属于展示层。它驱动底部 `SessionFooter` 的连接状态、活动指示器、输入提交能力和视觉样式，不进入 Provider 或会话历史。
...
- 按对话区、新输出提示、输入框、底部状态栏和尺寸提示的顺序组合全屏界面，并由 `Screen` 提供四边一格安全边距。
...
- `SessionFooter`：根选择器使用 `#session-footer`，模型、工作区和分支分别使用 `#footer-model`、`#footer-workspace`、`#footer-branch`；根据宽度先隐藏 Git 分支，再隐藏工作区，品牌、模型和连接状态保持可见。
...
现有 `wide ≥100`、`compact 72–99`、`narrow <72` 与 `too-small <48×14` 的外部终端尺寸断点保持不变。响应式类只改变 Footer 的字段可见性和极小尺寸提示，不改变上述节点顺序，也不移除正常尺寸下的外层安全边距。
...
- 错误处理完成后 Footer 保持 `ERROR`，Composer 重新可用；用户提交下一条消息后再进入 `UPLINKING`，成功完成后回到 `READY`。
...
└── tui/
    ...
    └── widgets/
        ...
        ├── chrome.py              # Footer、Welcome、Activity、NewOutput
...
├── test_tui_app.py                # 新建：Textual Pilot 端到端交互、节点顺序与外层边距
├── test_tui_widgets.py            # 新建：Footer 字段、输入、滚动、Markdown、响应式布局
...
| 主界面布局 | `Screen` 四边统一 `padding: 1`；对话区与输入区之后以单行 `SessionFooter` 收尾 | 内容不紧贴终端边缘，同时保证品牌、模型、工作区、分支和状态位于最底部；Footer 与 Composer 共用 surface 并避免双边框。 |
| 响应式断点 | 保持 `wide ≥100`、`compact 72–99`、`narrow <72`；低于 `48×14` 隐藏正常内容并显示尺寸提示 | 提供确定、可测试的降级规则，同时覆盖常见 80 列终端和分屏；布局降级不改变 Footer 的底部位置或正常尺寸下的安全边距。 |
...
| 测试策略 | 业务事件单测 + Textual Pilot 行为与计算样式断言 + 现有四张关键屏幕快照 | DOM 顺序和计算样式直接验证 Footer 位置、四边 padding 与断点隐藏规则；快照覆盖最终观感，但不把全部颜色或字符变化变成脆弱断言。 |
...
| 需求 | 设计归属 |
|---|---|
| F1 全屏单列布局 | `tui.app` 的固定 Compose 顺序、`Screen` 外层 padding、`cyberpunk.tcss` |
| F2 启动与会话信息 | `tui.metadata`、`SessionFooter`、`WelcomeCard` |
...
| F14 响应式布局 | Resize 处理、Footer 字段降级、外层安全边距、明确断点与 `too-small` CSS 状态类 |
