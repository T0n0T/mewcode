...
- T1–T42 描述的首版赛博朋克 TUI 已在提交 `2337a68` 完成。
- 本次已批准的布局修订只执行 T43–T52；T42 作为已满足的基线依赖，不重新执行 T1–T42。
- 修订仅涉及 Footer、布局 CSS、界面测试、快照和对应用户文档，不改变运行时、Provider、工具或配置语义。

## 初始里程碑文件清单（T1–T42，已完成）
...
| 新建 | `mewcode/tui/widgets/chrome.py` | 底部状态栏、欢迎卡和活动提示 |
...
2. 定义带四边一格安全边距的根布局、对话区、输入区、底部状态栏、卡片和弹层基础样式。
...
1. 实现 `SessionFooter` 的品牌、模型、工作区、分支和连接状态。
...
`uv run pytest tests/test_tui_widgets.py -q -k "footer or welcome"`
...
1. 按 Conversation、NewOutputIndicator、Composer、SessionFooter 和尺寸提示的顺序组合界面，并保留 Activity 与弹层入口。
...
2. 意外异常映射为无堆栈的通用错误。
3. 保留错误前已经显示的片段。
4. 错误后恢复 Composer，并将 Footer 保持为 ERROR；下一条成功回合再恢复 READY。
...
2. compact 隐藏分支、narrow 继续隐藏工作区，Footer 始终位于 Composer 下方。
3. 低于 48×14 隐藏正常界面、只显示尺寸提示且保留草稿。
4. 根据输出编码让全屏和纯文本模式切换 Unicode 与 ASCII 字形。
5. 检测 NO_COLOR 时应用单色类。
6. 覆盖 120×36、80×24、60×18 和 40×10，并断言正常尺寸的四边安全边距稳定。
...
1. 添加宽屏空白会话快照，确认 Footer 位于 Composer 下方且四边留白。
2. 添加 80 列流式回复与工具卡快照，确认 Footer 保持最底部。
3. 添加窄屏和 NO_COLOR 快照，确认字段降级和 Footer 样式正确。
...
1. 说明真实 TTY 自动进入带底部会话状态栏和外层安全边距的全屏界面。
...
| 操作 | 文件 | 职责 |
|---|---|---|
| 修改 | `mewcode/tui/widgets/chrome.py` | 将会话信息组件改为底部 `SessionFooter`，同步语义化选择器 |
| 修改 | `mewcode/tui/widgets/__init__.py` | 导出 `SessionFooter` 并移除旧名称 |
| 修改 | `mewcode/tui/app.py` | 调整 Compose 顺序并把活动状态路由到 Footer |
| 修改 | `mewcode/tui/cyberpunk.tcss` | 增加四边安全边距、Footer 样式和极小尺寸隐藏规则 |
| 修改 | `tests/test_tui_widgets.py` | 覆盖 Footer 字段、状态更新和语义化选择器 |
| 修改 | `tests/test_tui_app.py` | 覆盖节点顺序、计算样式、响应式隐藏和 Footer 位置 |
| 修改 | `tests/__snapshots__/test_tui_app/*.raw` | 更新现有四张关键布局快照 |
| 修改 | `README.md` | 将用户可见布局说明从 Header 更新为 Footer 与安全边距 |
| 修改 | `docs/03-cyberpunk-tui/{spec,plan,task,checklist}.md` | 保持本次需求、设计、任务和验收记录一致 |

## T43：先锁定布局契约的失败测试

**文件：** `tests/test_tui_widgets.py`、`tests/test_tui_app.py`
**依赖：** T42（已由提交 `2337a68` 满足）

**步骤：**

1. 将 Chrome 测试的预期组件改为 `SessionFooter`，并断言 `#footer-model`、`#footer-workspace`、`#footer-branch` 与连接状态。
2. 添加应用直接子节点顺序断言：Conversation、NewOutputIndicator、`#composer-shell`、SessionFooter、尺寸提示。
3. 添加正常尺寸计算样式与可见区域断言，要求 `Screen` 四边 padding 均为一格，Footer 位于 Composer 下方且底部仍有一格空隙。
4. 添加 compact、narrow 和 too-small 断言：依次隐藏分支、工作区；极小尺寸只显示尺寸提示且草稿保留。
5. 运行聚焦测试并记录预期失败；失败必须来自旧 Header、旧顺序或缺少外层 padding，不接受无关异常。

**验证：**

`uv run pytest tests/test_tui_widgets.py tests/test_tui_app.py -q -k "footer or compose_order or screen_padding or responsive"` 在实现前按上述契约失败。

## T44：把会话信息组件重命名为 Footer

**文件：** `mewcode/tui/widgets/chrome.py`、`mewcode/tui/widgets/__init__.py`、`mewcode/tui/app.py`、`tests/test_tui_widgets.py`、`tests/test_tui_app.py`
**依赖：** T43

**步骤：**

1. 将 `SessionHeader` 重命名为 `SessionFooter`，根 id 改为 `session-footer`。
2. 将模型、工作区和分支 id 分别改为 `footer-model`、`footer-workspace` 和 `footer-branch`，保留品牌与连接状态语义。
3. 同步 Widget 导出、应用导入和 `_set_activity()` 查询目标，不保留临时兼容别名。
4. 更新组件测试名称和查询，确认完整字段、缺失分支、长模型截断与活动状态仍正确。

**验证：**

`uv run pytest tests/test_tui_widgets.py tests/test_tui_app.py -q -k "footer or welcome or long_model"`

## T45：把 Footer 组合到 Composer 下方
...
1. 将 `CyberpunkChatApp.compose()` 固定为 ConversationView、NewOutputIndicator、`#composer-shell`、SessionFooter、`#size-warning` 的顺序。
2. 保持 ConversationView 使用剩余高度，使 Footer 成为正常尺寸下最后一个可见节点。
3. 保持欢迎卡、Composer 初始焦点、活动状态和尺寸提示行为不变。
4. 运行应用级顺序与活动状态测试，确认 Footer 更新不会改变回合语义。

**验证：**

`uv run pytest tests/test_tui_app.py -q -k "compose_order or mount or focus or activity"`

## T46：应用安全边距与 Footer 响应式样式

**文件：** `mewcode/tui/app.py`、`mewcode/tui/cyberpunk.tcss`、`mewcode/tui/widgets/chrome.py`、`tests/test_tui_app.py`、`tests/test_tui_widgets.py`
**依赖：** T45

**步骤：**

1. 为正常 `Screen` 设置四边统一 `padding: 1`，不在子组件中用负边距抵消。
2. 将所有 Header 选择器改为 Footer 选择器；Footer 使用单行 `$surface`，不添加上下边框。
3. 保留 Composer 顶部青色分隔线，使 Composer 与 Footer 形成连续 surface 且没有双边框。
4. 同步 ASCII 和 NO_COLOR 规则；compact 隐藏分支，narrow 继续隐藏工作区。
5. too-small 隐藏 Conversation、NewOutputIndicator、Composer 和 Footer，只显示尺寸提示；缩小后新到达的未读输出也不得让 NewOutputIndicator 重新出现。
6. 在 120×36、80×24、60×18 和 40×10 下验证位置、字段可见性、草稿和安全边距。

**验证：**

`uv run pytest tests/test_tui_widgets.py tests/test_tui_app.py -q -k "screen_padding or footer or responsive or no_color or ascii"`
...
1. 将全屏界面的 `session header` 改为位于 Composer 下方的 `session footer`。
2. 说明正常尺寸下内容与终端边缘保留一格安全边距。
3. 保持按键、纯文本回退和范围边界说明不变。

**验证：**

`rg -n "session footer" README.md && rg -n "one-cell|safe margin" README.md`

## T48：更新并检查四张关键快照

**文件：** `tests/test_tui_app.py`、`tests/__snapshots__/test_tui_app/*.raw`
**依赖：** T46

**步骤：**

1. 仅更新现有 wide welcome、80 列 tool turn、narrow welcome 和 NO_COLOR welcome 四张快照。
2. 逐张检查四边留白、Composer 与 Footer 顺序、Footer 最底部位置和断点字段隐藏。
3. 确认快照中没有计时值、临时路径、随机标识或意外控制字符变化。
4. 不借快照更新掩盖与本次布局无关的视觉回归。

**验证：**

`uv run pytest tests/test_tui_app.py -q -k snapshot --snapshot-update && uv run pytest tests/test_tui_app.py -q -k snapshot`

## T49：运行聚焦回归、全量测试与编译检查
...
1. 运行 Widget 与应用聚焦测试，确认布局、状态、输入、缩放和快照全部通过。
2. 运行完整测试集，确认 Provider、运行时、工具、CLI 和纯文本模式没有回归。
3. 编译 `mewcode` 与 `tests`，确认重命名没有遗漏导入或循环依赖。
4. 运行 `git diff --check` 并确认没有占位符、编码替换字符或无关文件。

**验证：**

`uv run pytest tests/test_tui_widgets.py tests/test_tui_app.py -q && uv run pytest && uv run python -m compileall mewcode tests && git diff --check`

## T50：验证 wheel、TTY 与纯文本启动路径

**文件：** 构建产物及启动日志，不新增仓库文件
**依赖：** T49

**步骤：**

1. 构建 wheel，并确认安装包仍包含 `mewcode/tui/cyberpunk.tcss`。
2. 通过伪 TTY 启动 `uv run mewcode`，在空 Composer 发送 Ctrl+D，确认全屏界面可进入并干净退出。
3. 将输入和输出重定向后发送 `quit`，确认走纯文本路径且日志不含 ANSI 控制序列。
4. 检查启动日志不包含 API key 或配置中的其他敏感值。

**验证：**

`uv build --wheel && unzip -l "$(ls -t dist/*.whl | head -n 1)" | rg "mewcode/tui/cyberpunk.tcss"`

并运行：

`printf '\004' | timeout 10s script -qfec 'uv run mewcode' /tmp/mewcode-tty.log && printf 'quit\n' | uv run mewcode > /tmp/mewcode-plain.log && uv run python -c "from pathlib import Path; assert b'\x1b' not in Path('/tmp/mewcode-plain.log').read_bytes()"`
...
1. 以提交 `2337a68` 为固定点运行 `code-review`，分别检查仓库标准与已批准 spec/plan 的符合性。
2. 修复所有确认的问题；若审查建议扩大 Agent、Provider、工具或配置范围则拒绝扩张。
3. 对每项修复重跑受影响的聚焦测试，再运行 `git diff --check`。
4. 搜索并移除生产代码、测试和 README 中遗留的 `SessionHeader`、Header 选择器与 `session header` 文案。

**验证：**

`! rg -n '\bSessionHeader\b|#session-header|#header-(model|workspace|branch)|session header' mewcode tests README.md && git diff --check`
...
1. 查看最终 diff 和状态，只暂存本次 Footer、padding、测试、快照及文档改动。
2. 创建一个聚焦的本地提交，不推送远端。
3. 确认提交后工作区干净，提交统计不包含配置、密钥、构建产物或无关文件。

**验证：**

`git show --stat --oneline --decorate HEAD && git status --short`
...
- **C9（T43–T52）：** Footer、外层安全边距、回归测试、快照与文档；双轴审查通过后创建一个本地提交，不推送。

## 自检结果

- plan.md 中的每个模块至少有一个对应任务；本次新增的 Compose 顺序、Footer 和安全边距分别由 T43–T48 覆盖。
- 每个任务均包含明确文件、依赖、步骤和验证命令。
- T1–T52 存在合法执行顺序，没有循环依赖；T42 的基线满足状态已经明确记录。
- `SessionFooter`、根选择器和字段选择器与 plan.md 保持一致，没有继续设计旧 Header 接口。
- T43 明确要求先观察契约失败，T44–T50 的验证则要求实现后全部通过。
