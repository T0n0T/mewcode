...
- [ ] 启动 MewCode 后进入行式交互界面，并显示猫猫文字画、配置摘要和 `╭─ you` 输入提示（验证：运行 `printf 'exit\n' | uv run mewcode`，观察启动输出）。
...
- [ ] 默认配置查找优先读取 `./.mewcode/config.yaml`，缺失时回退 `~/.mewcode/config.yaml`（验证：运行 `uv run pytest tests/test_config.py -k "prefers_local or falls_back"`）。
- [ ] YAML 配置能加载当前唯一配置，并校验 `name`、`protocol`、`model`、`base_url`、`api_key`、`thinking`（验证：运行 `uv run pytest tests/test_config.py`，期望合法配置和字段校验测试通过）。
...
- [ ] OpenAI 连接失败错误包含目标 URL 和检查 `base_url`/Provider 服务是否运行的提示（验证：运行 `uv run pytest tests/test_providers.py -k connection_url`）。
...
- [ ] `AGENTS.md` 少于 200 行，且包含本项目代码发现和验证约定（验证：运行 `wc -l AGENTS.md` 并阅读内容）。
- [ ] 改动范围符合任务文件清单（验证：运行 `git status --short`，确认新增/修改文件与 `docs/task.md` 文件清单一致）。
...
- [ ] 场景 3：项目级和用户级配置都存在时启动，系统使用项目级配置（验证：运行 `uv run pytest tests/test_config.py -k prefers_local`）。
- [ ] 场景 4：配置文件缺失时启动，终端显示包含查找路径的可读错误，进程返回非零（验证：临时指定空 HOME 或移走配置后运行 `uv run python -m mewcode`，观察错误和退出码）。
- [ ] 场景 5：同一会话连续提问两次，第二次请求携带第一轮用户问题和助手回复（验证：使用 fake provider 记录收到的 messages，运行两轮输入，检查第二轮历史包含前一轮完整对话）。
