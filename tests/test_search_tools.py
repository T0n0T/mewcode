...
async def test_glob_files_rejects_escape_pattern(tmp_path: Path):
    result = await execute(
        tmp_path, GlobFilesTool(), ToolCall("1", "glob_files", {"pattern": "../*.py"})
    )
...
async def test_glob_path_segments_do_not_cross_directories(tmp_path: Path):
    ...
    direct = await execute(
        tmp_path, GlobFilesTool(), ToolCall("1", "glob_files", {"pattern": "src/*.py"})
    )
...
@pytest.mark.asyncio
async def test_read_only_search_tools_can_overlap_without_shared_results(
    tmp_path: Path,
):
    ...
    python_files, text_files = await asyncio.gather(
        ...
        execute(
            tmp_path, GlobFilesTool(), ToolCall("2", "glob_files", {"pattern": "*.txt"})
        ),
    )
